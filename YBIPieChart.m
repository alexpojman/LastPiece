...
#pragma mark - Touch Handling (Selection Notification)
...
- (void)setSliceSelectedAtPoint:(CGPoint)selectionPoint
{
    for (int index = 0; index < [_pieView.layer.sublayers count]; index++) {
        // check if layer.position is equal to CGPoint
        SliceLayer *layer = [_pieView.layer.sublayers objectAtIndex:index];
        CGFloat currX = layer.position.x;
        double middleAngle = (layer.startAngle + layer.endAngle)/2.0;
        NSLog(@" x is %f, y is %d. middle angle is %f", currX, (int)layer.position.y, middleAngle);
    }
}
